...
class Job:
    ...
    def __init__(self,prefix,num,content,registry,arg_string):
        self.num = num
        self.prefix = prefix
        ...
        self.cluster_command = 'qsub -cwd %s -e %s -o %s ./%s' %\
            (self.arg_string, self.error_file, self.out_file, self.script_file)
...
def submit_and_complete_jobs(script_content_list,prefix='script',use_cluster=True, arg_string=''):
    global USE_CLUSTER
    ...
    for n, content in enumerate(script_content_list):
        ...
        job = Job(prefix=prefix,num=n,content=content,
                registry=job_list, arg_string=arg_string)
